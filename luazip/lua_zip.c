...
static int S_archive_open(lua_State* L) {
    ...
    int          flags = (lua_gettop(L) < 2) ? 0 : luaL_checkinteger(L, 2);
...
static int S_archive_name_locate(lua_State* L) {
    ...
    int          flags = (lua_gettop(L) < 3) ? 0 : luaL_checkinteger(L, 3);
    int          idx;
...
static int S_archive_stat(lua_State* L) {
    ...
    int             path_idx  = (lua_isnumber(L, 2)) ? luaL_checkinteger(L, 2)-1 : -1;
    int             flags     = (lua_gettop(L) < 3)  ? 0    : luaL_checkinteger(L, 3);
...
static int S_archive_get_name(lua_State* L) {
    struct zip** ar        = check_archive(L, 1);
    int          path_idx  = luaL_checkinteger(L, 2)-1;
    int          flags     = (lua_gettop(L) < 3)  ? 0    : luaL_checkinteger(L, 3);
    const char*  name;
