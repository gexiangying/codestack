...
function luacomE.FillTypeInfo(rawTypeInfo)
  ...
  -- copies flags
  foreach(attr.flags, function(i,v) typeinfo[i] = v end)
    ...
    for i = 0, num_methods - 1 do
      ...
      if rawmethod ~= nil then
        ...
        if method.parameters then
          ...
          foreachi(method.parameters, add_param)
  ...
  local function fillCoClass(interfaces, num_interfaces)
    ...
    for i = 0, num_interfaces - 1 do
      ...
      foreach(typeflags, function(i,v) interface[i] = v end)
...
function luacomE.DumpTypeLib(obj, htmlfile)
  ...
  local function describe_coclass(i, typeinfo)
    ...
    for i=1, getn(typeinfo.interfaces) do
  ...
  local function describe_enum(i, typeinfo)
    ...
    io.write("<ul>\n")
    foreachi(typeinfo.values, describe_constant)
    io.write("</ul>\n")
  end
  ...
  local function describe_interface(i, typeinfo)
    ...
    local function describe_method_use(method)
      ...
      if method.typeinv == "propget" then
        if method.num_params == 0 then
          ...
          io.write("com_obj:get" .. method.name .. "(")
          foreachi(method.parameters, param_lister())
          io.write(")")
        end
      elseif method.typeinv == "propput" then
        if method.num_params == 1 then
          ...
          if method.type ~= "void" then
            io.write("retval")
            foreachi(method.parameters, retval_lister(false))
            io.write(" = ")
          else
            local lister, checker = retval_lister(true)
            foreachi(method.parameters, lister)
    ...
    io.write("<ul>\n")
    foreachi(typeinfo.methods, describe_method)
    io.write("</ul>\n")
  end
...
function interface_proto:Write(file)
  ...
  local function param_writer()
    ...
    return function(i, param)
        ...
        if param.attributes then
          file:write("[")
          foreach(param.attributes, attribute_writer(false))
          file:write("] ")
        end
  ...
  local function property_writer(i, property)
    ...
    foreach(property.attributes, attribute_writer(true))
    file:write("\n  ]\n  ")
  ...
  local function method_writer(i, method)
    ...
    foreach(method.attributes, attribute_writer(true))
    file:write("\n  ]\n  ")
    if method.type then
    ...
    if method.parameters then
      file:write("\n    ")
      foreachi(method.parameters, param_writer())
      file:write("\n  );\n")
    else
      file:write("void" .. ");\n")
    end
  ...
  if self.attributes then
    file:write("[\n  ")
    foreach(self.attributes, attribute_writer(true))
    file:write("\n]\n")
  end
...
--  table.foreachi(self.typedefs, typedef_writer)
    file:write("\n  properties:\n")
    foreachi(self.properties, property_writer)
    start_id = getn(self.properties)
    file:write("\n  methods:\n")
    foreachi(self.methods, method_writer)
  end
  file:write("\n};\n\n")
end
...
function coclass_proto:Write(file)
  ...
  local function interface_writer(i, interface)
    ...
    interface.name = nil
    file:write("[")
    foreach(interface, attribute_writer(false))   
    file:write("] ")
  ...
  foreachi(self.interfaces, interface_writer)
  file:write("};\n\n")
end
...
function library_proto:WriteODL(filename)
  ...
  foreachi(self.imports, import_writer)
  foreachi(self.typedefs, typedef_writer)
  foreachi(self.interfaces, interface_writer)
  foreachi(self.coclasses, coclass_writer)
  file:write("};\n\n")
  file:close()
end
